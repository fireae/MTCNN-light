...
Pnet::Pnet() {
  ...
  this->conv1_matrix = new Blob;
  this->conv1 = new Blob;
  this->maxPooling1 = new Blob;

  this->maxPooling_matrix = new Blob;
  this->conv2 = new Blob;

  this->conv3_matrix = new Blob;
  this->conv3 = new Blob;

  this->score_matrix = new Blob;
  this->score_ = new Blob;

  this->location_matrix = new Blob;
  this->location_ = new Blob;
  ...
  long dataNumber[13] = {conv1, 10, 10,      conv2, 16,      16, conv3,
                         32,    32, conv4c1, 2,     conv4c2, 4};
  Dtype *pointTeam[13] = {this->conv1_wb->pdata,    this->conv1_wb->pbias,
                          this->prelu_gmma1->pdata, this->conv2_wb->pdata,
                          this->conv2_wb->pbias,    this->prelu_gmma2->pdata,
                          this->conv3_wb->pdata,    this->conv3_wb->pbias,
                          this->prelu_gmma3->pdata, this->conv4c1_wb->pdata,
                          this->conv4c1_wb->pbias,  this->conv4c2_wb->pdata,
                          this->conv4c2_wb->pbias};
  // string filename = "Pnet.txt";
  // readData(filename, dataNumber, pointTeam);

  readData(model_weights_PNet_,
           sizeof(model_weights_PNet_) / sizeof(model_weights_PNet_[0]),
           dataNumber, pointTeam, sizeof(dataNumber) / sizeof(dataNumber[0]));
}
Pnet::~Pnet() {
  freeBlob(this->rgb);
  freeBlob(this->conv1);
  freeBlob(this->maxPooling1);
  freeBlob(this->conv2);
  freeBlob(this->conv3);
  freeBlob(this->score_);
  freeBlob(this->location_);

  freeBlob(this->conv1_matrix);
  freeWeight(this->conv1_wb);
  freepRelu(this->prelu_gmma1);
  freeBlob(this->maxPooling_matrix);
  freeWeight(this->conv2_wb);
  freeBlob(this->conv3_matrix);
  freepRelu(this->prelu_gmma2);
  freeWeight(this->conv3_wb);
  freeBlob(this->score_matrix);
  freepRelu(this->prelu_gmma3);
  freeWeight(this->conv4c1_wb);
  freeBlob(this->location_matrix);
...
void Pnet::run(Mat &image, float scale) {
  if (firstFlag) {
    ...
    feature2MatrixInit(this->maxPooling1, this->maxPooling_matrix,
                       this->conv2_wb);
    convolutionInit(this->conv2_wb, this->maxPooling1, this->conv2,
                    this->maxPooling_matrix);
...
void Pnet::generateBbox(const struct Blob *score, const struct Blob *location,
                        Dtype scale) {
  // for pooling
  int stride = 2;
  int cellsize = 12;
  int count = 0;
  // score p
  Dtype *p = score->pdata + score->width * score->height;
  Dtype *plocal = location->pdata;
  struct Bbox bbox;
      ...
      if (*p > Pthreshold) {
        ...
        for (int channel = 0; channel < 4; channel++)
          bbox.regreCoord[channel] =
              *(plocal + channel * location->width * location->height);
...
Rnet::Rnet() {
  ...
  this->rgb = new Blob;
  this->conv1_matrix = new Blob;
  this->conv1_out = new Blob;
  this->pooling1_out = new Blob;

  this->conv2_matrix = new Blob;
  this->conv2_out = new Blob;
  this->pooling2_out = new Blob;

  this->conv3_matrix = new Blob;
  this->conv3_out = new Blob;

  this->fc4_out = new Blob;
  ...
  long dataNumber[16] = {conv1, 28,  28,  conv2, 48,    48, conv3,    64,
                         64,    fc4, 128, 128,   score, 2,  location, 4};
  Dtype *pointTeam[16] = {
      this->conv1_wb->pdata,   this->conv1_wb->pbias, this->prelu_gmma1->pdata,
      ...
      this->score_wb->pdata,   this->score_wb->pbias, this->location_wb->pdata,
      this->location_wb->pbias};
  // string filename = "Rnet.txt";
  // readData(filename, dataNumber, pointTeam);
  readData(model_weights_RNet_,
           sizeof(model_weights_RNet_) / sizeof(model_weights_RNet_[0]),
           dataNumber, pointTeam, sizeof(dataNumber) / sizeof(dataNumber[0]));
  // Init the network
...
Rnet::~Rnet() {
  freeBlob(this->rgb);
  freeBlob(this->conv1_matrix);
  freeBlob(this->conv1_out);
  freeBlob(this->pooling1_out);
  freeBlob(this->conv2_matrix);
  freeBlob(this->conv2_out);
  freeBlob(this->pooling2_out);
  freeBlob(this->conv3_matrix);
  freeBlob(this->conv3_out);
  freeBlob(this->fc4_out);
  freeBlob(this->score_);
  freeBlob(this->location_);
...
void Rnet::RnetImage2MatrixInit(struct Blob *Blob) {
  Blob->channel = 3;
  Blob->height = 24;
  Blob->width = 24;
  ...
  Blob->pdata = (Dtype *)malloc(Blob->channel * Blob->height * Blob->width *
                                sizeof(Dtype));
  if (Blob->pdata == NULL) cout << "the image2MatrixInit is failed!!" << endl;
  memset(Blob->pdata, 0,
...
Onet::Onet() {
  ...
  this->conv3_matrix = new Blob;
  this->conv3_out = new Blob;
  this->pooling3_out = new Blob;

  this->conv4_matrix = new Blob;
  this->conv4_out = new Blob;

  this->fc5_out = new Blob;

  this->score_ = new Blob;
  this->location_ = new Blob;
  this->keyPoint_ = new Blob;
  ...
  long dataNumber[21] = {conv1, 32,    32,    conv2,    64,  64,       conv3,
                         64,    64,    conv4, 128,      128, fc5,      256,
                         256,   score, 2,     location, 4,   keyPoint, 10};
  Dtype *pointTeam[21] = {this->conv1_wb->pdata,    this->conv1_wb->pbias,
                          this->prelu_gmma1->pdata, this->conv2_wb->pdata,
                          this->conv2_wb->pbias,    this->prelu_gmma2->pdata,
                          this->conv3_wb->pdata,    this->conv3_wb->pbias,
                          this->prelu_gmma3->pdata, this->conv4_wb->pdata,
                          this->conv4_wb->pbias,    this->prelu_gmma4->pdata,
                          this->fc5_wb->pdata,      this->fc5_wb->pbias,
                          this->prelu_gmma5->pdata, this->score_wb->pdata,
                          this->score_wb->pbias,    this->location_wb->pdata,
                          this->location_wb->pbias, this->keyPoint_wb->pdata,
                          this->keyPoint_wb->pbias};
  // string filename = "Onet.txt";
  // readData(filename, dataNumber, pointTeam);
  readData(model_weights_ONet_,
           sizeof(model_weights_ONet_) / sizeof(model_weights_ONet_[0]),
           dataNumber, pointTeam, sizeof(dataNumber) / sizeof(dataNumber[0]));
  // Init the network
...
Onet::~Onet() {
  ...
  freeBlob(this->conv3_matrix);
  freeBlob(this->conv3_out);
  freeBlob(this->pooling3_out);
  freeBlob(this->conv4_matrix);
  freeBlob(this->conv4_out);
  freeBlob(this->fc5_out);
  freeBlob(this->score_);
  freeBlob(this->location_);
  freeBlob(this->keyPoint_);
...
void Onet::OnetImage2MatrixInit(struct Blob *Blob) {
  Blob->channel = 3;
  Blob->height = 48;
  Blob->width = 48;
...
mtcnn::mtcnn(int row, int col) {
  ...
  int count = 0;
  for (vector<float>::iterator it = scales_.begin(); it != scales_.end();
       it++) {
    if (*it > 1) {
...
mtcnn::~mtcnn() { delete[] simpleFace_; }
...
void mtcnn::findFace(Mat &image) {
  ...
  for (size_t i = 0; i < scales_.size(); i++) {
    ...
    nms(simpleFace_[i].boundingBox_, simpleFace_[i].bboxScore_,
  ...
  // second stage
  count = 0;
  for (vector<struct Bbox>::iterator it = firstBbox_.begin();
       it != firstBbox_.end(); it++) {
    if ((*it).exist) {
      ...
      if (*(refineNet.score_->pdata + 1) > refineNet.Rthreshold) {
        memcpy(it->regreCoord, refineNet.location_->pdata, 4 * sizeof(Dtype));
  ...
  // third stage
  count = 0;
  for (vector<struct Bbox>::iterator it = secondBbox_.begin();
       it != secondBbox_.end(); it++) {
    if ((*it).exist) {
      Rect temp((*it).y1, (*it).x1, (*it).y2 - (*it).y1, (*it).x2 - (*it).x1);
      Mat thirdImage;
      ...
      if (*(outNet.score_->pdata + 1) > outNet.Othreshold) {
        memcpy(it->regreCoord, outNet.location_->pdata, 4 * sizeof(Dtype));
        it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
  ...
  for (vector<struct Bbox>::iterator it = thirdBbox_.begin();
       it != thirdBbox_.end(); it++) {
    if ((*it).exist) {
      rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2),
                Scalar(0, 0, 255), 2, 8, 0);
      for (int num = 0; num < 5; num++)
        circle(image,
               Point((int)*(it->ppoint + num), (int)*(it->ppoint + num + 5)), 3,
               Scalar(0, 255, 255), -1);
    }
  }
  firstBbox_.clear();
